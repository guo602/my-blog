


!!! star "生平"
    时间线履历

<!-- !!! tip "2022"
    发表第一篇论文

!!! success "2023"
    加入中国海洋石油集团工作

!!! star "2025"
    申请入党 
    
!!! note "2025"
    申请入党
    -->






<!-- <link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/Wcowin/Wcowin.github.io@main/docs/about/sty/portfolio.css"> -->

<!-- 
<script src="https://code.iconify.design/iconify-icon/1.0.7/iconify-icon.min.js"></script> -->

<link rel="stylesheet" href="../styles/portfolio.css">

<!-- 
<div class="qualification__tabs">
    <div class="qualification__button qualification__active" data-target='#education'>
        <iconify-icon icon="fluent:hat-graduation-12-regular" class="qualification__icon"></iconify-icon>
        生平
    </div>
</div> -->
<div class="qualification__data title-no-counter" >
    <div></div>
    <div>
        <span class="qualification__rounder"></span>
        <span class="qualification__line"></span>
    </div>
    <div>
        <h3 class="qualification__title">和平中心小学</h3>
        <span class="qualification__subtitle">毕业</span>
        <div class="qualification__calendar">
            <iconify-icon icon="tabler:calendar" aria-hidden="true"></iconify-icon>
            <span>2009</span>
        </div>
    </div>
</div>
<div class="qualification__data title-no-counter">
    <div>
        <h3 class="qualification__title">天津一中</h3>
        <span class="qualification__subtitle">理科</span>
        <div class="qualification__calendar">
            <iconify-icon icon="tabler:calendar" aria-hidden="true"></iconify-icon>
            <span>2009 - 2015</span>
        </div>
    </div>
    <div>
        <span class="qualification__rounder"></span>
        <span class="qualification__line"></span>
    </div>
</div>
<div class="qualification__data title-no-counter">
    <div></div>
    <div>
        <span class="qualification__rounder"></span>
        <span class="qualification__line"></span>
    </div>
    <div>
        <h3 class="qualification__title">南开大学</h3>
        <span class="qualification__subtitle">软件工程 学士</span>
        <div class="qualification__calendar">
            <iconify-icon icon="tabler:calendar" aria-hidden="true"></iconify-icon>
            <span>2015 - 2019</span>
        </div>
    </div>
</div>
<div class="qualification__data title-no-counter">
    <div>
        <h3 class="qualification__title">新东方北美</h3>
        <span class="qualification__subtitle">国际学科讲师</span>
        <div class="qualification__calendar">
            <iconify-icon icon="tabler:calendar" aria-hidden="true"></iconify-icon>
            <span>2019 - 2021</span>
        </div>
    </div>
    <div>
        <span class="qualification__rounder"></span>
        <span class="qualification__line"></span>
    </div>
</div>

<div class="qualification__data title-no-counter">
    <div></div>
    <div>
        <span class="qualification__rounder"></span>
        <span class="qualification__line"></span>
    </div>
    <div>
        <h3 class="qualification__title">南开大学</h3>
        <span class="qualification__subtitle">软件工程 硕士</span>
        <div class="qualification__calendar">
            <iconify-icon icon="tabler:calendar" aria-hidden="true"></iconify-icon>
            <span>2022 - 2025</span>
        </div>
    </div>
</div>
<div class="qualification__data title-no-counter">
    <div>
        <h3 class="qualification__title">腾讯北京</h3>
        <span class="qualification__subtitle">云计算开发</span>
        <div class="qualification__calendar">
            <iconify-icon icon="tabler:calendar" aria-hidden="true"></iconify-icon>
            <span>2023 - 2024</span>
        </div>
    </div>
    <div>
        <span class="qualification__rounder"></span>
        <span class="qualification__line"></span>
    </div>
</div>

<div class="qualification__data title-no-counter">
    <div></div>
    <div>
        <span class="qualification__rounder"></span>
        <span class="qualification__line"></span>
    </div>
    <div>
        <h3 class="qualification__title">中国海油</h3>
        <span class="qualification__subtitle">专业技术</span>
        <div class="qualification__calendar">
            <iconify-icon icon="tabler:calendar" aria-hidden="true"></iconify-icon>
            <span>2025 - 现在</span>
        </div>
    </div>
</div>

<!-- <HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#608DBD,direction:145,strength:15)" width="100%" color=#608DBD SIZE=1> -->

<HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#608DBD,direction:135,strength:12)" width="100%" color=#3F52B5 SIZE=1>



